...
class dataGrid implements control
{
    ...
    function display($retorno = false)
    {
        ...
        $salida .= "<table  class='table table-striped table-bordered table-hover' ><thead>";
        ...
        for($x=0; $x < $this->dataTable->getNumFields(); $x++)
        {
			...
			{
           		$salida .= "<th style='width:$ancho'>$valor</th>";
        	}
		}
       $salida .= "
...
class label implements control 
{
	...
	function __toString()
	{
		...
		return "<label id='$this->id' style='$style' type='text'>$this->text</label>";
	}
	...
	function __construct($text = "")
	{
		...
		$this->height = 30;
		$this->text = $text;
		$this->fontSize = (defined('PANTALLA_FUENTE'))?PANTALLA_FUENTE:14;
	}
...
class multiSelect implements control
{
	...
	function getValue()
	{
		...
		$this->selectValue = (isset($_POST[$this->id.'multi']))?implode('|', $_POST[$this->id.'multi']):$this->selectValue;
	...
	function __toString()
	{
		...
		$control .= "<div id='div{$this->id}'>";
		$valor = explode("|",$this->selectValue);
		for($x=0; $x < $this->dataSource->getNumRows(); $x++)
		{
			...
			$selected = (in_array($row[0], $valor))?" checked ":"";
			
			$control .= "<label title='{$title}'>
					<input type='checkbox' name='{$this->id}multi[]' value='{$row[0]}' {$selected}/>
					{$row[1]}
			</label><br/>";
		}
...
class fcombo implements control{
	public $id;
	public $fecha;
	public $clase;	
	public $y_mayor;
	...
	function getValue(){
		sanarFcombo('txt'.$this->id);
		return (isset($_POST['txt'.$this->id]))?$_POST['txt'.$this->id]:'';
	}
	
	function __construct($id, $fecha = "", $clase='requerido'){
				$this->id=$id;
				$this->fecha = $fecha;
				$this->clase = $clase;
				$this->y_mayor = date("Y",time());
				$this->y_menor = $this->y_mayor - 40;
				$this->type = 'text';
	}				
	
	function setValue($fecha){
		$this->fecha = $fecha;
	}
	...
	function __toString()
	{
		...
		$d = new comboBox("{$nombre}D", new dataTable($ddata));
		$d->class = $this->clase;
		$d->setValue($vd);
		
		
		
		$td = new dataTable("SELECT id, nombre FROM meses ORDER BY id");
		$m =  new comboBox("{$nombre}M", $td);
		$m->class = $this->clase;
		$m->setValue($vm);
		
		
		$dx = array("val"=>array(),"txt"=>array());
		for($x=$this->y_mayor; $x > $this->y_menor; $x--)
		{
			$dx["val"][]=$x;
			$dx["txt"][]=$x;
		}
		$y =  new comboBox("{$nombre}Y", new dataTable($dx));
		$y->class = $this->clase;
		$y->setValue($vy);
...
class button implements control 
{
	...
	public function setValue($texto){
		$this->texto = $texto;
	}
